...
void calendar(){
  ...
  if (ble_rx_buffer_len) {
    char *text;
    memcpy(text, ble_rx_buffer + 1, ble_rx_buffer_len - 1);
    text[ble_rx_buffer_len - 1] = '\0';
    ...
    //ble_rx_buffer_len = 0;//clear afer reading
    delay(1000);//should catch input
    lib_aci_send_data(PIPE_UART_OVER_BTLE_UART_TX_TX, (uint8_t*)"9", (uint8_t)2);
  } else if (ble_rx_buffer) {
    lib_aci_send_data(PIPE_UART_OVER_BTLE_UART_TX_TX, (uint8_t*)"7", (uint8_t)2);
  } else {
    lib_aci_send_data(PIPE_UART_OVER_BTLE_UART_TX_TX, (uint8_t*)"8", (uint8_t)2);
  }
